...
#' @param delimiter A character to use for delimiting values.
#'
#' @rdname layouts
#' @aliases csv_log_layout
#' @export
csv_log_layout <- function(delimiter = ",", time_format = "%Y-%m-%d %H:%M:%S") {
  ...
  # TODO: Support for NA characters and escaping.

  function(level, ...) {
    fields <- paste(..., sep = delimiter, collapse = delimiter)
    time_fmt <- fmt_current_time(time_format)
    sprintf("%s%s%s%s%s\n", time_fmt, delimiter, level, delimiter, fields)
  }
}
